...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    ...
    next_layer = hidden_layer1
    for i in range(1, n_layers):
      next_layer = tf.contrib.layers.fully_connected(
        next_layer,
        size,
        activation_fn=tf.nn.relu)
    out = tf.contrib.layers.fully_connected(next_layer,
                                              output_size,
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    #n_chan_config = state_shape[2] * config.state_history
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.update_baseline_op = tf.losses.mean_squared_error(self.baseline, self.baseline_target_placeholder) # TODO
    #######################################################
    #########          END YOUR CODE.          ############
